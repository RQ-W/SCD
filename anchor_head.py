...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def reg_two_stage_SRKD_distill(self, stu_reg, tea_reg, tea_cls, stu_cls, gt_truth, img_metas):
        def select_single_mlvl(mlvl_tensors, batch_id, detach=True):
            assert isinstance(mlvl_tensors, (list, tuple))
            num_levels = len(mlvl_tensors)
        ...
        def calculate_area(bboxes, W, H):
            """计算归一化后的预测框面积"""
            width = bboxes[:, 2] - bboxes[:, 0]
            height = bboxes[:, 3] - bboxes[:, 1]
            area = width * height
            # 归一化面积
            area_normalized = area / (W * H)
        ...
        def giou_loss(pred: Tensor, target: Tensor, eps: float = 1e-7) -> Tensor:
            ...
            Return:
                Tensor: Loss tensor.
            """
            gious = bbox_overlaps(pred, target, mode='iou', is_aligned=True, eps=eps)
            loss = 1 - gious
            return loss
        ...
        anchor_list, _ = self.get_anchors(featmap_sizes, img_metas)
        reg_total_loss = torch.tensor(0.).cuda()
        cls_total_loss = torch.tensor(0.).cuda()
        ...
        for i in range(len(gt_truth)):
            ...
            cls_loss = F.binary_cross_entropy(tea_rpn_cls_pred, stu_rpn_cls_pred, reduction='none')
            reg_loss = giou_loss(stu_rpn_bbox_pred_decode, tea_rpn_bbox_pred_decode)
            if reg_loss.numel() != 0:
                area1 = calculate_area(stu_rpn_bbox_pred_decode, 1024, 1024)
                area2 = calculate_area(tea_rpn_bbox_pred_decode, 1024, 1024)
                # weight = torch.abs(tea_rpn_cls_pred - stu_rpn_cls_pred)
                # reg_loss = (reg_loss * weight).sum() / weight.sum()
                omega = np.exp(-(area1.cpu().numpy() + area2.cpu().numpy()) / 2) + 1
                reg_loss = reg_loss.cpu().numpy()
                reg_loss = reg_loss * omega
                reg_loss = reg_loss.mean()
                reg_total_loss += reg_loss
            if cls_loss.numel() != 0:
                area1 = calculate_area(stu_rpn_bbox_pred_decode, 1024, 1024)
                ...
                cls_loss = cls_loss * omega
                cls_loss = cls_loss.mean()
                cls_total_loss += cls_loss

        return reg_total_loss, cls_total_loss
